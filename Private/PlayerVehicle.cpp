...
APlayerVehicle::APlayerVehicle()
{
	...
	GasTankTrigger->SetCollisionResponseToAllChannels(ECR_Overlap);
	GasTankTrigger->SetCollisionResponseToChannel(ECC_Visibility, ECR_Block);

	DoorTrigger = CreateDefaultSubobject<UBoxComponent>(TEXT("CarTrigger"));
	DoorTrigger->SetupAttachment(GetMesh());
	DoorTrigger->SetBoxExtent(GasTankTriggerExtent);
	DoorTrigger->SetCollisionResponseToAllChannels(ECR_Overlap);
	DoorTrigger->SetCollisionResponseToChannel(ECC_Visibility, ECR_Block);
...
void APlayerVehicle::ApplyThrottle(const FInputActionValue& Value)
{
	...
	if (bIsRefueling) // Refueling movement lockout
	{
	...
	if (ThrottleValue > 0.0f && !bIsRefueling) // W Moving forward
	{
		VehicleMovement->SetThrottleInput(ThrottleValue);
		VehicleMovement->SetBrakeInput(0.0f);
	}
	else if (ThrottleValue < 0.0f && !bIsRefueling) // S moving backwards
	{
		if (IsMovingForward())
		{
			// s acts as a brake
...
void APlayerVehicle::UpdateSpeedometr()
{
	if (VehicleMovement && WindowSpeedMesh)
	{
		...
		// transferring speed to the pointer
...
void APlayerVehicle::EnterVehicle(ADetectiveCharacter* const PlayerCharacter)
{
	...
	if (PC)
	{
		// hide the player, transition to the vehicle
...
void APlayerVehicle::ExitVehicle(const FInputActionValue& Value)
{
	...
	if (PC)
	{
		...
		//calculating the player spawn point
...
void APlayerVehicle::Interact(AActor* const Interactor, UPrimitiveComponent* const HitComponent)
{
	...
	{
		if (bIsRefueling && ConnectedPump) { ConnectedPump->TogglePistolet(); }
		else { ToggleGasCover(); }
		return;
	}
	else if (HitComponent == DoorTrigger)
	{
		if (ADetectiveCharacter* const Player = Cast<ADetectiveCharacter>(Interactor)) { EnterVehicle(Player); }
		return;
	}

	if (GasTankTrigger->IsOverlappingActor(Interactor))
	{
		if (bIsRefueling && ConnectedPump) { ConnectedPump->TogglePistolet(); }
		else { ToggleGasCover(); }
		return;
	...
	if (DoorTrigger->IsOverlappingActor(Interactor))
	{
		if (ADetectiveCharacter* const Player = Cast<ADetectiveCharacter>(Interactor)) { EnterVehicle(Player); }
		return;
	}
